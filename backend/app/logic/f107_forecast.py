...
def point_forecast(history: DailySeries, lead_days: int) -> np.ndarray:
    ...
    for i in range(lead_days):
        ...
        f = x0 + (target - x0) * w
        out[i] = max(f, settings.f107_floor_sfu)
    return out
...
def quantile_bands(
    ...
    for i in range(lead_days):
        ...
        for q in _QUANTILES:
            offset = eq.get(q, 0.0)
            bands[q][i] = max(point[i] + offset, settings.f107_floor_sfu)
...
def forecast(
    ...
    point = point_forecast(history, lead_days)
    bands = quantile_bands(point, error_quantiles)
